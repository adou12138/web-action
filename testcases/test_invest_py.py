...
@pytest.mark.all
@pytest.mark.usefixtures('my_class_invest')
class TestInvest:
    '这是测试投资的用例'
    ...
    @pytest.mark.investfailed
    @pytest.mark.parametrize('data', invest.bid_error_pop)
    # @data(*invest.bid_error_pop)
    def test_bid_failed1(self, data, my_class_invest):
        # IndexPage(self.driver).bid()

        driver, login_page, invest_page = my_class_invest
        invest_page.bid(data['amount'])
        invest_page.click_bid_button_element()

        try:
            # self.assertTrue(data['expected'] in self.invest_page.bid_error_pop_text_element())
            assert data['expected'] in invest_page.bid_error_pop_text_element()
            print('Test Pass!')
    ...
    @pytest.mark.investfailed
    @pytest.mark.parametrize('data', invest.bid_error_pop_button)
    # @data(*invest.bid_error_pop_button)
    def test_bid_failed2(self, data, my_class_invest):
        ...
        try:
            ...
            assert data['expected'] in invest_page.bid_error_pop_button_text_element()
            print('Test Pass!')
        except AssertionError as e:
            print('Test Failed!!! AssertionError：{}'.format(e))
            raise e
    ...
    @pytest.mark.investsuccess
    @pytest.mark.parametrize('data', invest.bid_success_pop)
    # @data(*invest.bid_success_pop)
    def test_bid_success(self, data, my_class_invest):
        # 首页点击投标
        # IndexPage(self.driver).bid()
        # 投标页面输入投标
        driver, login_page, invest_page = my_class_invest
        before_amount = round(float(invest_page.data_amount()))
        # print(before_amount)
        ...
        try:
            ...
            assert data['expected'] in invest_page.bid_success_pop_text_element()
            print('Test Pass!')
        except AssertionError as e:
            print('Test Failed!!! AssertionError：{}'.format(e))
            raise e
        ...
        # 需要验证投资的金额 投资后的金额判断
        after_amount = round(float(invest_page.data_amount()))
        # print(after_amount)
        try:
            # self.assertTrue(before_amount - after_amount == int(data['amount']))
            assert before_amount - after_amount == int(data['amount'])
            print('Test Pass!')
        except AssertionError as e:
            print('Test Failed!!! AssertionError：{}'.format(e))
            raise e